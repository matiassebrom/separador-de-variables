...
@Component({
	selector: 'app-step3-separate-by',
	standalone: true,
	imports: [
		CommonModule,
		FormsModule,
	...
	templateUrl: './step3-separate-by.component.html',
	styleUrl: './step3-separate-by.component.scss'
})

export class Step3SeparateByComponent implements OnChanges {
	...
	onContinueClick() {
		...
		if (!fileId || headers.length === 0) {
			this.errorMessage.set('Selecciona al menos una columna para separar');
			return;
		}
		this.isSaving.set(true);
		this.apiService.setHeadersToSplit(fileId, headers).subscribe({
			next: (count) => {
			...
			error: (error) => {
				this.isSaving.set(false);
				this.errorMessage.set('Error al guardar columnas para separar');
				console.error('Error al hacer setHeadersToSplit:', error);
			}
		});
	}
